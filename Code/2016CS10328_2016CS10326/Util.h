...
double swappedScore(int *paper1, int *paper2, double oldScore, double **distanceMatrix, double c, Conference *conference)
{
    ...
    int numParallelTracks = conference->getParallelTracks();
    int numPapersInSession = conference->getPapersInSession();

    for (int i = 0; i < numParallelTracks; i++)
    {
        for (int j = 0; j < numPapersInSession; j++)
        {
            ...
            {
                newScore -= c * distanceMatrix[paperWith1][paper1InDM];
                newScore += c * distanceMatrix[paperWith1][paper2InDM];
            }
            ...
            {
                newScore -= c * distanceMatrix[paperWith2][paper2InDM];
                newScore += c * distanceMatrix[paperWith2][paper1InDM];
            }
        }
    }
